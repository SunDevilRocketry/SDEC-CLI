...
class SensorSentry:
    ...
    def poll(self,
        ...
        try:
            #Loop Forever
            while True:
                # Call dump function + dictionary creation
                poll_dump = self.dump(serial_connection)
                filtered_dump = {}

                # Put given sensors in dictonary 
                for sensor, reading in poll_dump.items():
                    if sensor.name in sensors or sensor.short_name in sensors:
                        filtered_dump[sensor] = reading
                yield filtered_dump
                time.sleep(0.2)
